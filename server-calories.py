...
API_KEY = os.getenv("OPENAI_API_KEY") #we want to hide the key, passwords etc from sharing git etc. this is safe way. 
...
def upload():
    ...
    # declare variables ford is_vitamins prompts and JSON
    # default starter value is empty string. only if boolean is tre
    is_vitamins_prompt = ""
    is_minerals_prompt = ""
    is_vitamins_json = ""
    is_minerals_json = ""
    # if boolean is true, set prompts:
    if is_vitamins:
        ...
        is_vitamins_json = """ "vitamins": {
            ...
            "E": { "mg": "milligrams of vitamin E", "pct_rda": "percent RDA of vitamin E" },

        },"""

    if is_minerals:
        is_minerals_prompt = "Also provide mineral information as milligrams and percent of RDA. Minerals to include are iron, copper, zinc, magnesium, among others. The exact minerals are in the provided JSON structure. "
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages = [
                {
                    "role": "system",
                    "content": f"""You are a dietician. A user sends you an image of a meal and you reply with a detailed analysis of the meal, including a restaurant menu-syle name and description of the meal, total calories in the meal, the number of calories per food item in the meal, and a breakdown of macronutrients, as grams of protein, fat and carbohydrates. Also give some concise prep instructions as a string of text. The calories per food item should be provided as a list of key-value pairs, with the key being the name. {is_vitamins_prompt} {is_minerals_prompt} Here is the precise JSON format that you need to use: 
                    {{
